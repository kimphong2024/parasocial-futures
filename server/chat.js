...
- Quotation marks are a claim to have reproduced a source's exact words. Quote ONLY the QUOTABLE lines, copied exactly (you may quote a contiguous part of one), in straight double quotes immediately followed by the citation. The text after each signal's dash is the classifier's paraphrase, not the source: quoting it will be rejected. Never put that summary, or your own words, inside quotation marks — every quotation is checked word-for-word against the retained source after you answer, and one that does not match is removed. Where a source's own phrasing carries the point, one exact quotation is worth more than a paraphrase.
