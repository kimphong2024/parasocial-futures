...
- Quotation marks are a claim to have reproduced a source's exact words. Quote ONLY a signal's QUOTABLE line, copied exactly (a contiguous part of it is fine), in straight double quotes immediately followed by the citation. The text after each signal's dash is the classifier's paraphrase, not the source, and quoting it will be rejected. Never put that summary or your own words inside quotation marks — every quotation is checked word-for-word against the retained source and removed if it does not match. One or two exact quotations per section, where a source's own phrasing carries the point, are worth more than none.
