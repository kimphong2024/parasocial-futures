...
**The 3D hero (`/models/hero.glb`).** The landing page's hero is a 3D Janus head — a realistic human face (lifelike skin, pores, imperfections) and a smooth white porcelain synthetic face with gold seam lines, looking in opposite directions on one shared head — that turns from human to machine as the visitor scrolls. Pipeline (`scripts/gen-3d.sh`): turnaround views of the same character were generated with Leonardo Phoenix at a fixed seed (saved in `server/public/img/rodin-refs/`); the two consistent back-to-back profile views were then passed as multi-view references to **Rodin v2** through the Leonardo API (`model: "rodin-v2"`, Quad mesh, medium quality, PBR materials, GLB output). The mesh took three human-reviewed iterations to get right — earlier attempts were rejected for noisy geometry and for opening on the back of the head rather than a face. The model renders in the browser via Google's `<model-viewer>` component with user controls disabled — the page's scroll position owns the camera, turning the head from the human profile through a half-and-half frontal to the porcelain face before the dive. If the model or its viewer fails to load, or the visitor prefers reduced motion, the original 2D hero image is shown instead.
