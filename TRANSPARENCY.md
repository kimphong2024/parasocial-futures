...
Each scenario carries its summary and all four CLA layers — litany, systemic, worldview, myth — rather than a single myth line. The layers are the method the scenario set is built on and were previously invisible on this page; they are open by default and foldable, because the scenario set is legitimately the longest thing in the report.
