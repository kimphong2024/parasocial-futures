...
**A retired 3D experiment (`/models/hero.glb`).** A 3D version of the hero was built and later rolled back in favour of the 2D image. The mesh — a Janus head with a realistic human face and a white porcelain synthetic face looking in opposite directions — was made by generating turnaround views of one character with Leonardo Phoenix at a fixed seed (`scripts/gen-3d.sh`, refs in `server/public/img/rodin-refs/`), then fusing the two consistent back-to-back profile views with **Rodin v2** through the Leonardo API (`model: "rodin-v2"`, Quad mesh, medium quality, PBR materials, GLB output). It went through three human-reviewed iterations and shipped briefly with a scroll-driven camera (human profile → half-and-half frontal → porcelain face), before the human decided the still image served the page better. The pipeline and mesh remain in the repository as a record of the attempt.
